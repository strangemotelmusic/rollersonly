            ...
            {(sire || dam) && (
              <div style={{ marginBottom: 48 }}>
                ...
                <div style={{ display: "flex", gap: 16, overflowX: "auto", paddingBottom: 8 }}>
                  <PedNode name={bird.name || "This bird"} ring={bird.ring_number} note="Subject" gold />
                  <span style={{ color: "var(--gold)", alignSelf: "center", flexShrink: 0 }}>›</span>
                  <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                    {sire && <PedNode name={sire.name || "Sire"} ring={sire.ring_number} note="Sire" />}
                    {dam && <PedNode name={dam.name || "Dam"} ring={dam.ring_number} note="Dam" />}
                  </div>
                </div>
              </div>
            )}
...
function PedNode({ name, ring, note, gold }: { name: string; ring: string | null; note: string; gold?: boolean }) {
  return (
    <div style={{ background: gold ? "rgba(212,175,55,0.05)" : "var(--surface)", border: `0.5px solid ${gold ? "var(--gold)" : "var(--border)"}`, padding: "10px 14px", borderRadius: 2, minWidth: 160, borderLeft: gold ? "2px solid var(--gold)" : undefined }}>
      ...
      {ring && <div style={{ fontSize: 9, color: "var(--gold)", marginBottom: 2 }}>{ring}</div>}
